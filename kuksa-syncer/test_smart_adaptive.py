...
async def test_smart_adaptive_syncer():
    ...
    trace_vars_data = {
        ...
        # Mix of existing and non-existing variables
        "trace_vars": ["ego_speed", "current_lane", "throttle_position", "non_existent_var", "brake_applied"],
        "duration": 5
    }
    
    print(f"🎯 Testing with mixed variables:")
    print(f"   Existing: ego_speed, throttle_position, brake_applied")  
    ...
    try:
        ...
        if trace_events:
            ...
            has_ego_speed = 'ego_speed' in monitored_vars
            has_throttle = 'throttle_position' in monitored_vars
            
            print(f"   ❌ current_lane filtered out: {not has_current_lane}")
            print(f"   ❌ non_existent_var filtered out: {not has_non_existent}")
            print(f"   ✅ ego_speed monitored: {has_ego_speed}")
            print(f"   ✅ throttle_position monitored: {has_throttle}")
            
            success = (not has_current_lane and not has_non_existent and 
                      has_ego_speed and len(monitored_vars) > 0)
